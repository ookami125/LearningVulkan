...
bool VulkanDevice::Init(VulkanInstance * vulkanInstance, HWND hwnd)
{
	...
	// GPU
	uint32_t numGPUs = 0;
	vkEnumeratePhysicalDevices(*vulkanInstance, &numGPUs, VK_NULL_HANDLE);
	if (numGPUs == 0)
	{
	...
	vkEnumeratePhysicalDevices(*vulkanInstance, &numGPUs, pGPUs.data());
	gpu = pGPUs[0];
	...
	if (numQueueFamily == 0)
	{
		printf("ERROR: No Queue Families were found!\n");
		return false;
	}
	...
	result = vkCreateWin32SurfaceKHR(*vulkanInstance, &win32SurfaceCI, VK_NULL_HANDLE, &surface);
	if (result != VK_SUCCESS)
	{
		printf("ERROR: Couldn't create Win32 Surface!\n");
		return false;
	}
	...
	{
		printf("ERROR: Couldn't find a graphics queue family index!\n");
		return false;
	}
	...
	if (result != VK_SUCCESS)
	{
		printf("ERROR: Couldn't get surface formats!\n");
		return false;
	}
...
void VulkanDevice::Unload(VulkanInstance * vulkanInstance)
{
	...
	vkDestroySurfaceKHR(*vulkanInstance, surface, VK_NULL_HANDLE);
}
